...
#define PORTB_DS      16
#define PORTE_LATCH   26
#define PORTA_CLOCK   5
...
void set_address_bus(unsigned long address)
{
	...
	byte b0 = address & 0x000000ff;
	byte b1 = address & 0x0000ff00;
	byte b2 = address & 0x00ff0000;
	byte b3 = address & 0xff000000;

	//disable latch line
	//bitClear(PORTE,PORTE_LATCH);
...
void writesd()
{
  ...
  while (Serial.available() > 0)
  {
    ...
    {
      filename.trim();
      Serial.println("Writing "+filename+" to Flash");

      char fnchar[50];
      filename.toCharArray(fnchar, 50);
      filename = "";
      File datafile = SD.open(fnchar);
      if (datafile)
      {
        unsigned long i = 0;
        while (datafile.available())
        {
            fast_write(i,datafile.read());
            i++;
        }
        datafile.close();
      }
...
void printDirectory(File dir, int numTabs) {
   while(true) {
     ...
     if (entry.isDirectory()) {
       ...
       Serial.print("\t\t");
       Serial.println(entry.size()*8, DEC);
     }
     entry.close();
   }
}
